...
// PromotionalConsiderations: optional setting to unlock wallet for block minting only;
...
// PromotionalConsiderations: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // PromotionalConsiderations: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // PromotionalConsiderations: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // PromotionalConsiderations: not avatar mode
                        {
...
// PromotionalConsiderations: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    // The following split & combine thresholds are important to security
    // Should not be adjusted if you don't understand the consequences
    static unsigned int nStakeSplitAge = (60 * 60 * 12);
...
// PromotionalConsiderations: check 'spent' consistency between wallet and txindex
// PromotionalConsiderations: fix wallet spent state according to txindex
...
// PromotionalConsiderations: disable transaction (only for coinstake)

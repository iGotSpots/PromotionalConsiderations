...
// Copyright (c) 2011-2013 The PromotionalConsiderations developers
...
const string strMessageMagic = "PromotionalConsiderations Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // PromotionalConsiderations: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // PromotionalConsiderations: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // PromotionalConsiderations: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// PromotionalConsiderations: find block wanted by given orphan block
...
// PromotionalConsiderations: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge, int nHeight)
{
	int64 nSubsidy = ((nHeight * nCoinAge) % 50) * COIN;
	return nSubsidy;
}
...
// PromotionalConsiderations: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // PromotionalConsiderations: target change every block
    // PromotionalConsiderations: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // PromotionalConsiderations: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // PromotionalConsiderations: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // PromotionalConsiderations: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // PromotionalConsiderations: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // PromotionalConsiderations: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // PromotionalConsiderations: track money supply and mint amount info
    ...
    // PromotionalConsiderations: fees are not collected by miners as in bitcoin
    // PromotionalConsiderations: fees are destroyed to compensate the entire network
...
// PromotionalConsiderations: total coin age spent in transaction, in the unit of coin-days.
...
// PromotionalConsiderations: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // PromotionalConsiderations: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // PromotionalConsiderations: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // PromotionalConsiderations: record proof-of-stake hash value
    ...
    // PromotionalConsiderations: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // PromotionalConsiderations: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // PromotionalConsiderations: coinbase output should be empty if proof-of-stake block
    ...
    // PromotionalConsiderations: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // PromotionalConsiderations: check that the block satisfies synchronized checkpoint
    ...
    // PromotionalConsiderations: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // PromotionalConsiderations: check proof-of-stake
    ...
    // PromotionalConsiderations: verify hash target and signature of coinstake tx
    ...
    // PromotionalConsiderations: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // PromotionalConsiderations: getblocks may not obtain the ancestor block rejected
    ...
    // PromotionalConsiderations: if responsible for sync-checkpoint send it
...
// PromotionalConsiderations: sign block
...
// PromotionalConsiderations: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "PromotionalConsiderations", wxOK | wxPROMON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "PromotionalConsiderations", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // PromotionalConsiderations: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // PromotionalConsiderations: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // PromotionalConsiderations: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // PromotionalConsiderations: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // PromotionalConsiderations: if detected invalid checkpoint enter safe mode
    ...
    // PromotionalConsiderations: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // PromotionalConsiderations: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // PromotionalConsiderations: record my external IP reported by peer
        ...
        // PromotionalConsiderations: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // PromotionalConsiderations: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // PromotionalConsiderations: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // PromotionalConsiderations: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // PromotionalConsiderations: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // PromotionalConsiderations: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
